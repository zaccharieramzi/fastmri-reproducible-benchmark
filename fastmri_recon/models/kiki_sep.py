...
def kiki_sep_net(previous_net, multiply_scalar, input_size=(640, None, 1), n_convs=5, n_filters=16, noiseless=True, lr=1e-3, to_add='I', last=False):
    ...
    if previous_net is None:
        kspace = conv2d_complex(kspace_input, n_filters, n_convs, output_shape=input_size, res=False)
        output = kspace
    else:
